            ...
            <div class="col-md-6 col-lg-4">
                <div class="card custom-glass-card mb-4">
                    <div class="card-header custom-card-header">
                        <h6 class="card-title mb-0" style="color: #fff; font-weight: 700; font-size: 1.1rem;">🎮 ${p.personagem.nome}</h6>
                    </div>
                    <div class="card-body">
                        <div style="display: flex; gap: 20px; align-items: flex-start;">
                            <img src="${fotoUrl}" alt="" class="zoomable-image photo-narrator-view" style="border-radius: 50%; object-fit: cover; ${fotoStyle}" onerror="this.src='data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'; this.style.background='linear-gradient(135deg, #1a2a4e 0%, #667eea 100%)'">
                            <div style="flex-grow: 1; font-size: 1rem; color: #e0e0e0; line-height: 1.5;">
                                <div><strong style="color: #667eea; font-size: 0.85rem; text-transform: uppercase;">Raça:</strong> ${p.personagem.raca || '-'}</div>
                                <div><strong style="color: #667eea; font-size: 0.85rem; text-transform: uppercase;">Idade:</strong> ${p.personagem.idade || '-'}</div>
                                <div><strong style="color: #667eea; font-size: 0.85rem; text-transform: uppercase;">Nível:</strong> ${p.personagem.nivel || 0}</div>
                                <div style="display: flex; flex-direction: column;">
                                    <div><strong style="color: #667eea; font-size: 0.85rem; text-transform: uppercase;">Altura:</strong> ${p.personagem.altura || '-'}</div>
                                    <div><strong style="color: #667eea; font-size: 0.85rem; text-transform: uppercase;">Peso:</strong> ${p.personagem.peso || '-'}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer custom-card-footer border-0 d-flex gap-2">
                        <button class="btn btn-sm btn-primary w-100" onclick="viewPersonagem('${p.personagem.id}')" style="background: #667eea; border: none; padding: 8px; font-weight: 600;">👁️ Ver Ficha</button>
                        <button class="btn btn-sm btn-outline-danger" onclick="removerJogador('${p.id}')" style="border-color: rgba(255,68,68,0.5); color: #ff4444; padding: 8px;">🗑️</button>
                    </div>
                </div>
            </div>
            `;
...
function registrarHistoricoCampanha(entry) {
    ...
    // Prevenir duplicados (timestamp exato) mas atualizar o DOM
